...
const AddPost = ({ onPostAdded }) => {
    ...
    const [isExpanded, setIsExpanded] = useState(false); // State for toggling read more/less
...
> {imageBlobUrl && <img src={imageBlobUrl} alt="Post Preview" className='post-preview-image' />}
          ...
          {isExpanded ? content || 'No content available.' : content.length > 100 ? `${content.substring(0, 100)}...` : content}
        </p>
        {content.length > 100 && (
      ...
      {articleUrl && (
        <p>
            <a href={articleUrl} target="_blank" rel="noopener noreferrer">
               ...
               <p><strong>Category:</strong> {category || 'No category provided.'}</p>
